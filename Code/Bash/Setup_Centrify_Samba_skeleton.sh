...
#rm /var/centrify/samba/adbindproxy.pl.rsp
...
attshare=$(grep -c "/path/s" /etc/samba/smb.conf)

if [ $attshare == 0 ]; 	then 
	echo "[INFO] Backing up Samba config to /etc/samba/smbd.conf.$(date +%F)"
...
if [ $attshare -gt 0 ]; then
	echo "[INFO] A share is already configured for the path /path/s - please check it is configured as follows in /etc/samba/smb.conf:"
	echo "
	[ATTACHMENTS]
		comment="attachments"
		path=/path/s
