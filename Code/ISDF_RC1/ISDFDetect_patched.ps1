...
if ($mode -eq 'Cloud' -and -not [string]::IsNullOrWhiteSpace($webhookUrl)) {
    ...
    try {
        if (-not [string]::IsNullOrWhiteSpace($thumb)) {
            # PS 5.1 safe: WebRequestSession w/ certificate by thumbprint
            $cert = Get-ChildItem -Path Cert:\LocalMachine\My,$env:USERPROFILE'\AppData\Roaming\Microsoft\SystemCertificates\My\Certificates' `
                -Recurse -ErrorAction SilentlyContinue | Where-Object { $_.Thumbprint -eq $thumb } | Select-Object -First 1
            if ($null -eq $cert) { throw "Client certificate not found: $thumb" }

            $session = New-Object Microsoft.PowerShell.Commands.WebRequestSession
            [void]$session.Certificates.Add($cert)
            $resp = Invoke-RestMethod -Method POST -Uri $webhookUrl -WebSession $session -Body $payload -ContentType 'application/json'
        } else {
            # Unauthenticated webhook (not recommended)
            $resp = Invoke-RestMethod -Method POST -Uri $webhookUrl -Body $payload -ContentType 'application/json'
        }

        if ($resp) {
            $ISDF.ISDF_WebhookLastOk = $true
            if ($resp.remote) {
                if ($resp.remote.ea1) { $ISDF.ISDF_EA1_Matches = ([string]::Equals([string]$resp.remote.ea1,$channel,'InvariantCulture')) }
                if ($resp.remote.ea2) { $ISDF.ISDF_EA2_Matches = ([string]::Equals([string]$resp.remote.ea2,$ea2Blob,'InvariantCulture')) }
