...
## Date:                20/12/13
##
## Description:         This script performs the RDF establish and split for the operators of ##REDACTED## group
##
## Usage:               ./Unisys_RDF.pl
##
## Version history:
## 1.0 - Initial release (adapted from Unisys_BCV.pl written by ##REDACTED##)
## 1.1 - Fix typo in symrdf verify (should be 'synchronized' not 'synched'
##
############################################################
...
#At this point in the procedure the operator is told to do a symdg -force delete on any DG that has 76 devices and BCVs
...
##
#Run a symrdf verify -split to see if the disks are in the correct split state
#Check the return code, valid return codes are 0 split, 25 partially split and 26 none are split
##

`/usr/symcli/bin/symrdf -g $DG_NAME verify -split`;
my $RC = $? >> 8;
...
##
#Run a symrdf establish command
##
`sleep 60`;
`/usr/symcli/bin/symrdf -g $DG_NAME establish -nop`;
my $EST_RC = $? >> 8;
...
##
#Run a symrdf verify -synchronized -i 120 -c 120
...
##
`sleep 60`;
`/usr/symcli/bin/symrdf -g $DG_NAME verify -synchronized -i 120 -c 120`;
my $VS_RC = $? >> 8;
...
#Run a symrdf split command, check the return code, 0 is success, >0 is fail
##
`sleep 60`;
`/usr/symcli/bin/symrdf -g $DG_NAME split -nop`;
my $SP_RC = $? >> 8;
