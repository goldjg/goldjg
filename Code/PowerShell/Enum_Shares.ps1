...
#$shares = net view ##REDACTED## | new-psobjectfrommatches -pattern '^(\S+)\s+((?:Print)|(?:Disk)).{6}\s+(.+)?' -Property $null,Name,Type,Comment

$shares = @()

01..25 | foreach {
            $server = ("##REDACTED##" + ("{0:D2}" -f $_))
            $lookup = net view $server
            If ($LASTEXITCODE -eq 2) {write-host ($Server + ": offline")} else {write-host ($Server + ": complete");$lookup|`
            new-psobjectfrommatches -pattern '^(\S+)\s+((?:Print)|(?:Disk)).{6}\s+(.+)?' -Property $null,Name,Type,Comment|`
            foreach {$obj = $null;
                     $obj = New-Object System.Object;
                     $obj | Add-Member -type NoteProperty -Name Share  -Value $_.Name;
...
$shares | export-csv -notype -path "K:\All_Shares.csv"