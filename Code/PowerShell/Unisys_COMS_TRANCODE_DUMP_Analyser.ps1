...
CREATE   TRANCODE   ABC               OF ABCDEF
AGENDA                   = T1ZYXINAG
,ANY_SC_LIST              = NONE    ,EVERY_SC_LIST            = NONE
,SECURITY_CATEGORY        = NONE    ,FUNCTION                 = 1
,TRANCODE_VALUE           = "ABC"
...
$Filtered = $Start|Select-String -Pattern "AGENDA","TRANCODE_VALUE" # extract only lines containing the string "AGENDA" or "TRANCODE_VALUE"
...
$Filtered|foreach {If ($_.ToString() -Match 'AGENDA'){
                   ...
                   If ($_.ToString() -Match 'TRANCODE_VALUE'){# found TRANCODE
                      $obj | Add-Member -type NoteProperty -name TRANCODE -Value ($_.ToString().SubString(28)).Replace('"','');    #Add TRANCODE property/value
...
$CSV|Select TRANCODE,AGENDA|Sort TRANCODE |Export-CSV -notype K:\TRANCODES.CSV    #sort the table, then export it to a CSV file
